...
class Settings_Page {
	...
	final public function register_settings(): void {
		...
		\add_settings_field(
			'custom_prompt',
			'<label for="air_custom_prompt"><span class="dashicons dashicons-editor-quote"></span> ' . \__( 'Custom Prompt', 'ai-image-renamer' ) . '</label>',
			array(
				$this,
	...
	final public function render_api_key_field(): void {
		...
		<p class="description">
			...
			if ( $saved ) :
				?>
				<?php \esc_html_e( 'Your Groq API key has been encrypted and saved. Type a new key to overwrite it.', 'ai-image-renamer' ); ?>
				<?php
			else :
				?>
