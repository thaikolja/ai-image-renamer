...
class Settings_Page {
	...
	final public function init(): void {
		...
		\add_action( 'admin_notices', array( $this, 'show_encryption_notice' ) );
		...
		\add_action( 'wp_ajax_air_dismiss_encryption_notice', array( $this, 'ajax_dismiss_encryption_notice' ) );
	}
	...
	private function prepare_models_for_template( array $models ): array {
		...
		foreach ( $models as $id => $data ) {
			$escaped[ esc_attr( (string) $id ) ] = array(
	...
	final public function enqueue_assets( string $hook ): void {
		...
		\wp_enqueue_style( 'air-admin', AIR_PLUGIN_URL . 'assets/css/admin.css', array(), (string) filemtime( AIR_PLUGIN_DIR . 'assets/css/admin.css' ) );

		\wp_enqueue_script( 'air-admin', AIR_PLUGIN_URL . 'assets/js/admin.js', array( 'jquery' ), AIR_VERSION, true );
		\wp_enqueue_script( 'air-admin-tabs', AIR_PLUGIN_URL . 'assets/js/admin-tabs.js', array( 'jquery' ), AIR_VERSION, true );
