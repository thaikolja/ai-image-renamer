...
final class Core {
	...
	public static function incrementCounter( $ctr, $inc ) {
		self::ensureTrue(
			self::ourStrlen( $ctr ) === self::BLOCK_BYTE_SIZE,
	...
	public static function HKDF( $hash, $ikm, $length, $info = '', $salt = null ) {
		...
		self::ensureTrue( self::ourStrlen( $prk ) >= $digest_length );
	...
	public static function hashEquals( $expected, $given ) {
		...
		// length.
		self::ensureTrue( self::ourStrlen( $expected ) === self::ourStrlen( $given ) );
	...
	public static function ourSubstr( $str, $start, $length = null ) {
		...
		if ( $exists ) {
			...
			if ( self::ourStrlen( $substr ) > $length || ( self::ourStrlen( $substr ) === 0 && $length !== 0 && $start !== $input_len ) ) {
				throw new Ex\EnvironmentIsBrokenException(
	...
	public static function pbkdf2(
		...
		if ( $raw_output ) {
			return (string) self::ourSubstr( $output, 0, $key_length );
		} else {
			return Encoding::binToHex( (string) self::ourSubstr( $output, 0, $key_length ) );
		}
	}
}
