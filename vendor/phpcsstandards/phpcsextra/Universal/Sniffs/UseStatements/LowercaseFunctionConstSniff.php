...
final class LowercaseFunctionConstSniff implements Sniff {
	...
	public function process( File $phpcsFile, $stackPtr ) {
		...
		$openGroup = $phpcsFile->findNext( array( \T_SEMICOLON, \T_CLOSE_TAG, \T_OPEN_USE_GROUP ), ( $stackPtr + 1 ) );
		if ( $openGroup === false || $tokens[ $openGroup ]['code'] !== \T_OPEN_USE_GROUP ) {
			// Not a group use statement.
			return;
		}

		$closeGroup = $phpcsFile->findNext( array( \T_SEMICOLON, \T_CLOSE_TAG, \T_CLOSE_USE_GROUP ), ( $openGroup + 1 ) );
	...
	public function processKeyword( File $phpcsFile, $stackPtr, $content ) {
		...
		$fix   = $phpcsFile->addFixableError( $error, $stackPtr, 'NotLowercase', array( $contentLC ) );
