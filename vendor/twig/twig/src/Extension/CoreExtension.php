...
final class CoreExtension extends AbstractExtension {
	...
	public const ARRAY_LIKE_CLASSES = array(
		'ArrayIterator',
		'ArrayObject',
		'CachingIterator',
	...
	public function getFilters(): array {
		return array(
			// formatting filters
			new TwigFilter( 'date', array( $this, 'formatDate' ) ),
			new TwigFilter( 'date_modify', array( $this, 'modifyDate' ) ),
			new TwigFilter( 'format', array( self::class, 'sprintf' ) ),
			new TwigFilter( 'replace', array( self::class, 'replace' ) ),
			new TwigFilter( 'number_format', array( $this, 'formatNumber' ) ),
			new TwigFilter( 'abs', 'abs' ),
			new TwigFilter( 'round', array( self::class, 'round' ) ),

			// encoding
			new TwigFilter( 'url_encode', array( self::class, 'urlencode' ) ),
			new TwigFilter( 'json_encode', 'json_encode' ),
			new TwigFilter( 'convert_encoding', array( self::class, 'convertEncoding' ) ),

			// string filters
			new TwigFilter( 'title', array( self::class, 'titleCase' ), array( 'needs_charset' => true ) ),
			new TwigFilter( 'capitalize', array( self::class, 'capitalize' ), array( 'needs_charset' => true ) ),
			new TwigFilter( 'upper', array( self::class, 'upper' ), array( 'needs_charset' => true ) ),
			new TwigFilter( 'lower', array( self::class, 'lower' ), array( 'needs_charset' => true ) ),
			new TwigFilter( 'striptags', array( self::class, 'striptags' ) ),
			new TwigFilter( 'trim', array( self::class, 'trim' ) ),
			new TwigFilter(
				'nl2br',
				array( self::class, 'nl2br' ),
				array(
					'pre_escape' => 'html',
					'is_safe'    => array( 'html' ),
				)
			...
			// array helpers
			new TwigFilter( 'join', array( self::class, 'join' ) ),
			new TwigFilter( 'split', array( self::class, 'split' ), array( 'needs_charset' => true ) ),
			new TwigFilter( 'sort', array( self::class, 'sort' ), array( 'needs_environment' => true ) ),
			new TwigFilter( 'merge', array( self::class, 'merge' ) ),
			new TwigFilter( 'batch', array( self::class, 'batch' ) ),
			new TwigFilter( 'column', array( self::class, 'column' ) ),
			new TwigFilter( 'filter', array( self::class, 'filter' ), array( 'needs_environment' => true ) ),
			new TwigFilter( 'map', array( self::class, 'map' ), array( 'needs_environment' => true ) ),
			new TwigFilter( 'reduce', array( self::class, 'reduce' ), array( 'needs_environment' => true ) ),
			new TwigFilter( 'find', array( self::class, 'find' ), array( 'needs_environment' => true ) ),

			// string/array filters
			new TwigFilter( 'reverse', array( self::class, 'reverse' ), array( 'needs_charset' => true ) ),
			new TwigFilter( 'shuffle', array( self::class, 'shuffle' ), array( 'needs_charset' => true ) ),
			new TwigFilter( 'length', array( self::class, 'length' ), array( 'needs_charset' => true ) ),
			new TwigFilter( 'slice', array( self::class, 'slice' ), array( 'needs_charset' => true ) ),
			new TwigFilter( 'first', array( self::class, 'first' ), array( 'needs_charset' => true ) ),
			new TwigFilter( 'last', array( self::class, 'last' ), array( 'needs_charset' => true ) ),

			// iteration and runtime
			new TwigFilter( 'default', array( self::class, 'default' ), array( 'node_class' => DefaultFilter::class ) ),
			new TwigFilter( 'keys', array( self::class, 'keys' ) ),
			new TwigFilter( 'invoke', array( self::class, 'invoke' ) ),
		);
	}

	public function getFunctions(): array {
		return array(
			new TwigFunction( 'parent', null, array( 'parser_callable' => array( self::class, 'parseParentFunction' ) ) ),
			new TwigFunction( 'block', null, array( 'parser_callable' => array( self::class, 'parseBlockFunction' ) ) ),
			new TwigFunction( 'attribute', null, array( 'parser_callable' => array( self::class, 'parseAttributeFunction' ) ) ),
			new TwigFunction( 'max', 'max' ),
			new TwigFunction( 'min', 'min' ),
			new TwigFunction( 'range', 'range' ),
			new TwigFunction( 'constant', array( self::class, 'constant' ) ),
			new TwigFunction( 'cycle', array( self::class, 'cycle' ) ),
			new TwigFunction( 'random', array( self::class, 'random' ), array( 'needs_charset' => true ) ),
			new TwigFunction( 'date', array( $this, 'convertDate' ) ),
			new TwigFunction(
				'include',
				array( self::class, 'include' ),
				array(
			...
			new TwigFunction(
				'source',
				array( self::class, 'source' ),
				array(
					'needs_environment' => true,
					'is_safe'           => array( 'all' ),
				)
			),
			new TwigFunction( 'enum_cases', array( self::class, 'enumCases' ), array( 'node_class' => EnumCasesFunction::class ) ),
			new TwigFunction( 'enum', array( self::class, 'enum' ), array( 'node_class' => EnumFunction::class ) ),
		);
	}

	public function getTests(): array {
		return array(
			new TwigTest( 'even', null, array( 'node_class' => EvenTest::class ) ),
			new TwigTest( 'odd', null, array( 'node_class' => OddTest::class ) ),
			new TwigTest( 'defined', null, array( 'node_class' => DefinedTest::class ) ),
			new TwigTest(
				'same as',
				null,
				array(
			...
			new TwigTest( 'none', null, array( 'node_class' => NullTest::class ) ),
			new TwigTest( 'null', null, array( 'node_class' => NullTest::class ) ),
			new TwigTest(
				'divisible by',
				null,
				array(
			...
			new TwigTest( 'constant', null, array( 'node_class' => ConstantTest::class ) ),
			new TwigTest( 'empty', array( self::class, 'testEmpty' ) ),
			new TwigTest( 'iterable', 'is_iterable' ),
			new TwigTest( 'sequence', array( self::class, 'testSequence' ) ),
			new TwigTest( 'mapping', array( self::class, 'testMapping' ) ),
			new TwigTest( 'true', null, array( 'node_class' => TrueTest::class ) ),
		);
	}
	...
	public function getExpressionParsers(): array {
		return array(
			...
			// binary operators
			new BinaryOperatorExpressionParser( ElvisBinary::class, '?:', 5, InfixAssociativity::Right, description: 'Elvis operator (a ?: b)', aliases: array( '? :' ) ),
	 ...
	 * @internal
	 */
	public static function include( Environment $env, $context, $template, $variables = array(), $withContext = true, $ignoreMissing = false, $sandboxed = false ): string {
	 ...
	 * @internal
	 */
	public static function getAttribute( Environment $env, Source $source, $object, $item, array $arguments = array(), $type = Template::ANY_CALL, $isDefinedTest = false, $ignoreStrictCheck = false, $sandboxed = false, int $lineno = -1 ) {
		...
		if ( Template::METHOD_CALL !== $type ) {
			...
			if ( $object instanceof \DateTimeInterface && \in_array( $item, array( 'date', 'timezone', 'timezone_type' ), true ) ) {
				if ( $isDefinedTest ) {
					return true;
				}
