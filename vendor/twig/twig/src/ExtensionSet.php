...
final class ExtensionSet {
	...
	private $testCallbacks = array();
	/** @var array<callable(string): (TokenParserInterface|false)> */
	private $parserCallbacks = array();
	...
	private function initExtensions(): void {
		...
		$this->dynamicFunctions  = array();
		$this->dynamicTests      = array();
		$this->visitors          = array();
	...
	private function initExtension( ExtensionInterface $extension ): void {
		...
		$expressionParsers = array();
		foreach ( $operators[0] as $operator => $op ) {
			$expressionParsers[] = new UnaryOperatorExpressionParser( $op['class'], $operator, $op['precedence'], $op['precedence_change'] ?? null, '', $op['aliases'] ?? array() );
		}
		foreach ( $operators[1] as $operator => $op ) {
			...
			if ( isset( $op['callable'] ) ) {
				$expressionParsers[] = $this->convertInfixExpressionParser( $op['class'], $operator, $op['precedence'], $op['associativity'], $op['precedence_change'] ?? null, $op['aliases'] ?? array(), $op['callable'] );
			} else {
				$expressionParsers[] = new BinaryOperatorExpressionParser( $op['class'], $operator, $op['precedence'], $op['associativity'], $op['precedence_change'] ?? null, '', $op['aliases'] ?? array() );
			}
		}
