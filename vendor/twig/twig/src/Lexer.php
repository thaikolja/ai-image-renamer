...
class Lexer {
	...
	private array $openingBrackets = array( '{', '(', '[' );
	private array $closingBrackets = array( '}', ')', ']' );
	...
	private const SPECIAL_CHARS = array(
		'f' => "\f",
		'n' => "\n",
		'r' => "\r",
		't' => "\t",
	...
	private function getOperatorRegex(): string {
		...
		foreach ( $this->env->getExpressionParsers() as $expressionParser ) {
			$expressionParsers = array_merge( $expressionParsers, array( $expressionParser->getName() ), $expressionParser->getAliases() );
		}
	...
	private function checkBrackets( string $code ): void {
		...
		if ( \in_array( $code, $this->openingBrackets, true ) ) {
			$this->brackets[] = array( $code, $this->lineno );
