...
final class EscaperRuntime implements RuntimeExtensionInterface {
	...
	public function setSafeClasses( array $safeClasses = array() ) {
		$this->safeClasses = array();
		$this->safeLookup  = array();
	...
	public function escape( $string, string $strategy = 'html', ?string $charset = null, bool $autoescape = false ) {
		...
		if ( ! \is_string( $string ) ) {
			...
			} elseif ( \in_array( $strategy, array( 'html', 'js', 'css', 'html_attr', 'url' ), true ) ) {
		...
		switch ( $strategy ) {
			...
			default:
				...
				$validStrategies = implode( '", "', array_merge( array( 'html', 'js', 'url', 'css', 'html_attr' ), array_keys( $this->escapers ) ) );
