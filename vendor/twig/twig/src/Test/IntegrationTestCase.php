...
abstract class IntegrationTestCase extends TestCase {
	...
	public function getTests( $name, $legacyTests = false ) {
		...
		foreach ( new \RecursiveIteratorIterator( new \RecursiveDirectoryIterator( $fixturesDir ), \RecursiveIteratorIterator::LEAVES_ONLY ) as $file ) {
			...
			$tests[ str_replace( $fixturesDir . '/', '', $file ) ] = array( str_replace( $fixturesDir . '/', '', $file ), $message, $condition, $templates, $exception, $outputs, $deprecation );
		}
		...
		foreach ( $outputs as $i => $match ) {
			...
			try {
				$prevHandler = set_error_handler(
					function ( $type, $msg, $file, $line, $context = array() ) use ( &$deprecations, &$prevHandler ) {
