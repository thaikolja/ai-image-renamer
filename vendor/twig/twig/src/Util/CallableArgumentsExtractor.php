...
final class CallableArgumentsExtractor {
	...
	public function extractArguments( Node $arguments ): array {
		...
		$extractedArgumentNameMap = array();
		$named                    = false;
				...
				if ( \count( $missingArguments ) ) {
					throw new SyntaxError(
						\sprintf(
							...
							implode( ', ', array_map( array( $this, 'toSnakeCase' ), $callableParameterNames ) ),
							\count( $missingArguments ) > 1 ? 's' : '',
							implode( '", "', $missingArguments )
						),
						$this->node->getTemplateLine(),
						$this->node->getSourceContext()
					);
				}
		...
		if ( $this->twigCallable->isVariadic() ) {
			$arbitraryArguments = $isPhpVariadic ? new VariadicExpression( array(), $this->node->getTemplateLine() ) : new ArrayExpression( array(), $this->node->getTemplateLine() );
	...
	private function toSnakeCase( string $name ): string {
		return strtolower( preg_replace( array( '/([A-Z]+)([A-Z][a-z])/', '/([a-z0-9])([A-Z])/' ), '\1_\2', $name ) );
	}
	...
	private function getCallableParameters(): array {
		...
		if ( $this->twigCallable->isVariadic() ) {
			...
			if ( $isArray && $argument->isDefaultValueAvailable() && array() === $argument->getDefaultValue() ) {
		...
		return array( $parameters, $isPhpVariadic );
	}
}
